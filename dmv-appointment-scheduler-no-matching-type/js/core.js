...
let resultData = [];
let searchArray = [];
const tableMaxLength  = 10; // LENGTH OF THE TABLE
let page = 0;
let maxPages = 0;
let resultLength = 0;
let searchBox = null;
...
const closingTime = (4) + 12; // INDEX (4) OF MILITARY TIME CLOSING TIME 5:00 PM, LEAVE AN HOUR FOR ONE LAST MEETING
...
window.addEventListener('DOMContentLoaded', function(){
        ...
        .finally(function(){
            fetch('./files/TellerData.json')
                ...
                .finally(function(){
                    ...
                    // SORT THE CUSTOMER AND TELLER DATA BY SPECIALTY SO THE SPECIALTY AND SERVICE TYPE DON'T MATCH FOR THE MOST PART
                    ...
                    for(let i = 0; i < customerDataLength;i++){
                        ...
                        while(count < customerDataLength && !valid){
                            if(tellerData[loop].SpecialtyType){
                                if(customerData[i].type){
                                    if(customerData[i].type !== tellerData[loop].SpecialtyType){
                                        ...
                                        //BEGIN DATE
                                        time = new Date((updateTime + (repeat * (maxSeparation * 1000 * 60))));
                                        ...
                                        //END DATE
                                        time = new Date(time.getTime() + (Number(customerData[i].duration) * 1000 * 60));
                                        ...
                                        const resObj = {
                                            ...
                                            duration: customerData[i].duration + ' min',
                                            tellerType: tellerData[loop].SpecialtyType,
                                            customerType: customerData[i].type,
                                        };
                            ...
                            loop++;
                            //RESET TELLER LOOP IF NEXT AVAILABLE TELLER IS NOT FOUND AND THE END OF THE LIST HAS BEEN REACHED
                            if(loop === tellerDataLength){
                                loop = 0;
    ...
    function searchBoxInput(){
        ...
        // SCAN EVERY OBJECT IN THE ARRAY
